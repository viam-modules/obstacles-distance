...
func TestObstacleDist(t *testing.T) {
	...
	srv, err := registerObstacleDistanceDetector(ctx, name, &inp, deps, nil)
	test.That(t, err, test.ShouldBeNil)
	test.That(t, srv.Name(), test.ShouldResemble, name)
	img, err := rimage.NewImageFromFile("detection_test.jpg")
	test.That(t, err, test.ShouldBeNil)
	...
	_, err = registerObstacleDistanceDetector(ctx, name, nil, deps, nil)
...
func TestRegistrationWithDefaultCamera(t *testing.T) {
	...
	// Create dependencies map
	deps := make(resource.Dependencies)
	deps[cameraName] = inject.NewCamera(cameraName.Name)

	service, err := registerObstacleDistanceDetector(ctx, modelName, &modelCfg, deps, nil)
	test.That(t, err, test.ShouldBeNil)
	...
	_, err = registerObstacleDistanceDetector(ctx, modelName, &modelCfg, deps, nil)
	test.That(t, err, test.ShouldNotBeNil)
